...
- (void)loadData
{
    ...
    for (int i = 1; i <= numberAd * 3; i++) {
        ...
        article.articleID = [NSString stringWithFormat:@"%d",i];
        article.title = RANDOM_STRING((rand() % 20) + 10);
        article.overview = RANDOM_STRING(30);
        article.sourceUrl = RANDOM_STRING(10);
        article.numberComment = [NSNumber numberWithInt:(rand() % 100)];
        article.site = RANDOM_STRING(5);
